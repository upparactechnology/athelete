...
export class ClientService {
    ...
    static async cancelBooking(bookingId, userId) {
        ...
        // Send FCM Notifications
        if (booking.user.fcm_token) {
            sendPushNotification(booking.user.fcm_token, "Booking Cancelled", `Your booking at ${booking.venue.name} has been cancelled. Policy outcome: ${refundMessage}`).catch(e => console.error("FCM error notifying user of cancelled booking:", e));
        }
        if (booking.venue.partner.fcm_token) {
            const slotTime = `${booking.slot.start_time} - ${booking.slot.end_time}`;
